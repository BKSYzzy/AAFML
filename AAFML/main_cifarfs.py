import  torch, os
import  numpy as np
from    MAML.cifarfs import CIFARFS
import  scipy.stats
...
def main():
    ...
    maml.set_dataset_config('cifarfs')
    ...
    cifarfs = CIFARFS('cifarfs', mode='train', n_way=args.n_way, k_shot=args.k_spt,
                        k_query=args.k_qry,
                        batchsz=int(args.epoch * args.client_num), resize=args.imgsz)
    cifarfs_test = CIFARFS('cifarfs', mode='test', n_way=args.n_way, k_shot=args.k_spt,
                             k_query=args.k_qry,
                             batchsz=6000, resize=args.imgsz)
    maml.load_state_dict(w_init)

    db = DataLoader(cifarfs, args.client_num, shuffle=False, num_workers=0, pin_memory=True)
    ...
    for step, (x_spt, y_spt, x_qry, y_qry) in enumerate(db):
        ...
        if step % 100 == 0 and step != 0:  # test
            db_test = DataLoader(cifarfs_test, 1, shuffle=False, num_workers=0, pin_memory=True)
            ...
            for x_spt, y_spt, x_qry, y_qry in db_test:
                ...
                attack_success_rate = maml.finetunning_test_cifar(x_spt, y_spt, x_qry,y_qry)  # meta.finetunning()
